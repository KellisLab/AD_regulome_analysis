## second round of doublet removal at the cluster level
...
# remove doublets
proj = proj[!(proj$Clusters %in% c("C18"))]
proj$Clusters.iter1 = proj$Clusters
proj$UMAP1.iter1 = proj@embeddings$UMAP$df[,1]
proj$UMAP2.iter1 = proj@embeddings$UMAP$df[,2]
...
saveRDS(proj,"ArchR.TSS6.2nd_doublets_rm.3rd_clustering.rds")
...
pdf("ArchR.TSS6.2nd_doublets_rm.3rd_clustering.pdf")
...
pdf("ArchR.TSS6.2nd_doublets_rm.3rd_clustering.cluster.QC.pdf")
...
library(gridExtra)
png("ArchR.TSS6.2nd_doublets_rm.3rd_clustering.by.cluster.png",width=28, height=28,units='in', res=300)
...
library(gridExtra)
png(paste0("ArchR.TSS6.2nd_doublets_rm.3rd_clustering.by.cluster.density.png"),width=28, height=28,units='in', res=300)
do.call("grid.arrange", c(p.list, nrow=5))
dev.off()
...
pdf(paste0("B.3.test/ArchR.TSS6.2nd_doublets_rm.3rd_clustering.MarkerGene.All.plot.umap.pdf"))
